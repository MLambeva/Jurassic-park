...
bool HelperFunctions::isLetter(char x)//Проверява дали символът е малка латинска буква.
{
	return x >= 'a' && x <= 'z';
}

bool HelperFunctions::isCorrectName(std::string name)//Проверява дали коректно е въведено името на динозавъра.
{
...
bool HelperFunctions::isCorrectGender(std::string gender)//Проверява дали коректно е въведен пола.
{
	return (gender.compare("male") == 0 || gender.compare("female") == 0);
}

bool HelperFunctions::isCorrectEra(std::string era)//Проверява дали коректно е въведена ерата.
{
	return (era.compare("Triassic") == 0 || era.compare("Cretaceous") == 0 || era.compare("Jura") == 0);
}

bool HelperFunctions::isCorrectOrder(std::string order)//Проверява дали коректно е въведен разредът на динозавъра.
{
	return (order.compare("herbivorous") == 0 || order.compare("carnivorous") == 0 || 
		    order.compare("flying") == 0 || order.compare("aqueous") == 0);
}

bool HelperFunctions::isCorrectSpecies(std::string species)//Проверява дали коректно е въведен видът на динозавъра.
{
...
bool HelperFunctions::isCorrectFood(std::string food)//Проверява дали коректно е въведена храната на динозавъра.
{
	return (food.compare("grass") == 0 || food.compare("meat") == 0 || food.compare("fish") == 0);
}

bool HelperFunctions::isCorrectAnimal(const Dinosaur& other)//Проверява дали коректно е въведен диназавъра.
{
...
bool HelperFunctions::isCorrectSize(std::string size)//Проверява дали коректно е въведен размерът на клетката.
{
	return (size.compare("small") == 0 || size.compare("medium") == 0 || size.compare("large") == 0);
}

bool HelperFunctions::isCorrectClimate(std::string climate)//Проверява дали коректно е въведен климатът на клетката.
{
